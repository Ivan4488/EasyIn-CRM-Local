...
import type { EmployeeCountEntry } from "./EmployeeCountListDropdown";
...
export const CompanyEmployeeCount = ({ className, id, error, disabled, tooltipText }: Props) => {
  ...
  const entries: EmployeeCountEntry[] = property?.jsonValue
    ? (Array.isArray(property.jsonValue) ? property.jsonValue as EmployeeCountEntry[] : [])
    : [];
  const primaryEntry = entries.find((e) => e.isPrimary);
  const stringValue = primaryEntry?.value?.toString() ?? property?.stringValue ?? "";
    ...
    >
      ...
      <div className="relative h-[38px]">
        ...
        <input
          ...
          readOnly
          onClick={() => {
            if (!isEditingLockType) rightMenuNavigationStore.setEmployeeCountModalPropertyId(id);
          }}
          value={isEditingLockType ? "Choose" : isValueHidden ? "" : stringValue}
          className={classNames(
            ...
            "block w-full border-0 bg-b1-black px-[12px] py-[8px] text-display-15 font-[400] text-white outline-none ring-1 ring-inset ring-gray-moderate transition placeholder:text-text-weak/40 focus:ring-1 focus:ring-inset"
          )}
        ...
        {propertiesStore.getShowMailbox() && (
