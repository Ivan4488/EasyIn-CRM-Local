...
import { DuplicateReviewsResponse } from "~/service/types";
...
import { useDuplicateConflictCount } from "./hooks/useDuplicateConflictCount";
import { useToast } from "~/components/UI/hooks/use-toast";

const selectorKey = "duplicateList";

const ConflictCountedRecord = ({
  review,
  onSelect,
}: {
  review: { id: string; source_contact_id: string; target_contact_id: string; matched_by: string; matched_value: string | null; status: "pending" | "merged" | "dismissed"; created_at: string };
  onSelect: () => void;
}) => {
  const conflictCount = useDuplicateConflictCount(review.source_contact_id, review.target_contact_id);
  return <DuplicateRecord review={review} conflictCount={conflictCount} onSelect={onSelect} />;
};

export const DuplicateListMiddle = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await axiosClient.get<DuplicateReviewsResponse>("/contacts/duplicate-reviews");
      return data;
    },
    staleTime: 30_000,
  });
  ...
  const handleBulkDismiss = async () => {
    ...
    try {
      await Promise.all(
        selected.map((item) =>
          axiosClient.post(`/contacts/duplicate-reviews/${item.id}/resolve`, {
            action: "dismiss",
          })
        )
      );
    ...
    try {
      await Promise.all(
        selected.map((item) => {
          ...
          return axiosClient.post(`/contacts/duplicate-reviews/${item.id}/resolve`, {
            action: "merge",
