...
import { useCompanyDuplicateReviewData } from "./useCompanyDuplicateReviewData";
import { useToast } from "~/components/UI/hooks/use-toast";
import { Loader } from "~/components/UI/Loader/Loader";
import { CompanyDuplicateLocationBar } from "./CompanyDuplicateLocationBar";
...
export const CompanyDuplicateReviewRightMenu = ({ reviewId }: Props) => {
  ...
  const {
    ...
    targetProperties,
    isLoading,
  } = useCompanyDuplicateReviewData(reviewId);
  ...
  const resolveMutation = useMutation({
    mutationFn: async (body: {
      ...
      axiosClient.post(`/companies/duplicate-reviews/${reviewId}/resolve`, body),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ["companyDuplicateReviews"] });
      ...
      setSelectedCompanyDuplicateReviewId(null);
    },
    onError: () => {
      toast({
        title: "Error",
  ...
  return (
    <div
      ...
      style={{ background: "rgba(29, 30, 32, 0.60)" }}
    >
      <CompanyDuplicateLocationBar
        name={targetCompany?.name ?? ""}
        avatar={targetCompany?.avatar ?? null}
        onBack={() => setSelectedCompanyDuplicateReviewId(null)}
      />
        ...
        {conflicts.length > 0 ? (
          ...
          <div className="px-[20px] py-[24px] flex flex-col gap-[8px]">
            ...
            <p className="text-display-13 text-text-weak leading-[1.5]">
              These two companies share no overlapping property values. You can
              safely merge them and no data will be lost.
            </p>
          </div>
        )}
      </Scrollbar>
